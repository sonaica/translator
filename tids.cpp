...
std::string& Value::name() { return name_; }

std::string& Value::type() { return type_; }
...
IdentifierTIDS::element::element(const element* parent = nullptr)
    : parent_(parent_), children_({}), variables_({}) {}
...
std::string IdentifierTIDS::element::check_id(const std::string& VariableName) {
    ...
    if (!find_result.first) {
        if (parent == nullptr) throw UndefinedVariable(VariableName);
        return parent->check_id(VariableName);
    }
    return find_result.second;
}

IdentifierTIDS::IdentifierTID() : cur_tid_(new element(nullptr)) {}

element*& IdentifierTIDS::cur_tid() { return cur_tid_; }
...
std::string FunctionTIDS::check_func_id(const std::string& func_name) {
    std::pair<bool, Function> find_result = name_set.find(func_name);
    if (!find_result.first) throw UndefinedFunction();
...
void FunctionTIDS::check_param_count(const std::string& func_name,
    ...
    std::pair<bool, Function> find_result = name_set.find(func_name);
    if (!func_result.first) throw UndefinedFunction();
    if (par_num < (int)find_result.second.argument_list().size())
        throw TooFewParameters(
            func_name, find_result.second.argument_list().size(), par_num);
}
...
void FunctionTIDS::push_func_par(const std::string& func_name,
    ...
    if (par_num.first) throw IdentifierAlreadyDefined();
...
void StructTIDS::push_id(const std::string& struct_name,
    ...
    if (!find_res.first) throw UndefinedStruct();
...
void StructTIDS::push_struct_id(const std::string& struct_name) {
    std::pair<bool, std::pair<FunctionTIDS, IdentifierTIDS>> find_res =
        name_set.find(struct_name);
    if (find_res.first) throw StructAlreadyDefined();
