...
void StructDefinition() {
    ...
    push_stack({ELEMENT_TYPE::OPERAND, variable_in_creation.type()});
    push_stack({ELEMENT_TYPE::OPERAND, variable_in_creation.name()});
    push_stack({ELEMENT_TYPE::MAKE_VARIABLE, ""});
    while (lexem.content == ",") {
        GetLexem();
        variable_in_creation.set_name(Name());
        IdTIDS.cur_tid()->push_id(
